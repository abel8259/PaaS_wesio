...
BEGIN
	...
	readln(key_priv0);
	writeln('|!custom value:(without 1s)');
	readln(key_custom0);
	writeln('>chave:',i0);
	cont:= cont + 1;
	...
	writeln('>',nome); 
	cont:= cont + 5;
	writeln('>complexity:', key_custom0,'WESRC');
	   	
	
	key_puv := key_pub0 + '-' + IntToStr(dg+i07);
	key_pruv := key_priv0 + '-' + IntToStr(i00+cont);
	writeln('>key_pub:',key_puv );
	writeln('>*******:',key_pruv);
	name_idt :=  'IDTSM'+key_puv+'.DAT';
	assign(A,name_idt);
	rewrite(A); 
        R.hsh := i0;
        R.nomeium := nome;
